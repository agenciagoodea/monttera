...
import { User, ShieldCheck, Download, ShoppingBag, MapPin, Lock, LogOut, Loader2, Check, AlertCircle, FileText } from 'lucide-react';
...
export default function MobileMyAccount() {
  ...
  return (
    <div className="flex flex-col gap-6 py-2">
      ...
      {/* Tabs de Navegação Mobile Estilo Abas Internas */}
      <div className="flex gap-2 border-b border-slate-100 pb-1 overflow-x-auto scrollbar-none -mx-4 px-4">
        {[
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 px-4 py-3 font-black text-[10px] uppercase tracking-wider transition-all border-b-2 flex items-center gap-1.5 ${
                isActive
      ...
      {/* Tab: Downloads (Matrizes Disponíveis) */}
      {activeTab === 'downloads' && (
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between border-b border-slate-50 pb-2">
            <h3 className="text-[11px] font-black uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
              <Download className="w-4 h-4 text-blue-500" /> Matrizes Compradas
            </h3>
            <span className="text-[9px] font-black bg-blue-50 text-blue-600 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
              {downloads.length} Disponíveis
            </span>
          </div>
          ...
          {downloads.length > 0 ? (
            <div className="flex flex-col gap-3.5">
              {downloads.map((item) => (
                <div key={item.id} className="bg-white rounded-3xl border border-slate-100 p-4 shadow-sm flex items-center gap-3">
            ...
            <div className="text-center py-16 bg-white rounded-[2.5rem] border border-slate-100 p-6 flex flex-col items-center">
              ...
              <p className="text-slate-400 font-medium text-[9px]">
                Seu download será liberado assim que seu pagamento for confirmado!
              </p>
            </div>
          )}
        </div>
      ...
      {/* Tab: Pedidos (Histórico de Compras) */}
      {activeTab === 'orders' && (
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between border-b border-slate-50 pb-2">
            <h3 className="text-[11px] font-black uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
              <ShoppingBag className="w-4 h-4 text-blue-500" /> Histórico de Pedidos
            </h3>
            <span className="text-[9px] font-black bg-blue-50 text-blue-600 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
          ...
          {orders.length > 0 ? (
            <div className="flex flex-col gap-3.5">
              {orders.map((order) => {
                ...
                return (
                  <div key={order.id} className="bg-white rounded-3xl border border-slate-100 p-4 shadow-sm flex flex-col gap-3">
      ...
      {/* Tab: Endereço de Faturamento */}
      {activeTab === 'address' && (
        <form onSubmit={handleUpdateAddress} className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm flex flex-col gap-4">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-2">
            <MapPin className="w-4 h-4 text-blue-600" /> Salvar Endereço
          </h3>
          ...
          <div className="flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 transition-all"
              />
            </div>

            {/* Rua */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Rua/Avenida</label>
              <input
                type="text"
            ...
            <div className="grid grid-cols-2 gap-3.5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 transition-all text-center"
                />
              </div>
            </div>
      ...
      {activeTab === 'profile' && (
        <div className="flex flex-col gap-6">
          {/* Dados Pessoais */}
          <form onSubmit={handleUpdateProfile} className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm flex flex-col gap-4">
            <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-2">
            ...
            <div className="flex flex-col gap-3.5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold focus:outline-none"
                />
              </div>
          ...
          {/* Segurança/Senha */}
          <form onSubmit={handleUpdatePassword} className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm flex flex-col gap-4">
            <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-2">

...
export default function MyAccount() {
  ...
  const handleUnlinkSocial = async (provider: string) => {
    setSocialMessage('');
    const confirmed = window.confirm(`Deseja desconectar sua conta ${provider === 'google' ? 'Google' : provider === 'facebook' ? 'Facebook' : 'Apple'}?`);
    if (!confirmed) return;
    setSocialLoading(true);
    try {
        ...
        <section className="space-y-6">
          ...
          {activeTab === 'social' && (
            <div className="bg-white rounded-[1.5rem] border border-slate-100 p-6 md:p-8 shadow-sm">
              ...
              {/* Apple */}
              {(() => {
                const appleAccount = socialAccounts.find((a) => a.provider === 'apple');
                return (
                  <div className="rounded-2xl border border-slate-100 p-5 mt-4 flex items-center justify-between gap-4 hover:border-slate-200 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-slate-800 text-sm">Apple</p>
                        {appleAccount ? (
                          <p className="text-xs text-slate-500 font-medium mt-0.5">
                            Conectado como {appleAccount.provider_email || 'conta vinculada'}
                          </p>
                        ) : (
                          <p className="text-xs text-slate-400 font-medium mt-0.5">Não conectado</p>
                        )}
                    ...
                    {appleAccount ? (
                      ...
                      <a
                        id="btn-link-apple"
                        href="/api/auth/apple?redirect=/minha-conta/contas-conectadas"
                        className="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors"
                      >
                        Conectar
                      </a>

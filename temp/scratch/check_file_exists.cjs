...
conn.on('ready', () => {
  console.log('SSH conectado. Verificando arquivos em /etc/nginx/...\n');

  const commands = [
    'ls -la /etc/nginx/nginx-includes.conf 2>&1',
    'ls -la /etc/nginx/ 2>&1 | head -n 40',
    'ls -la /etc/nginx/conf/ 2>&1 || true'
  ];

...
conn.on('ready', () => {
  console.log('SSH conectado. Localizando caminhos do Node.js no servidor...\n');

  const commands = [
    'which node 2>&1 || true',
    'node -v 2>&1 || true',
    'ls -la /opt/alt/ 2>&1 || true',
    'ls -la /usr/local/bin/node /usr/bin/node 2>&1 || true',
    'find /opt/alt/ -name "node" 2>/dev/null || true'
  ];

...
const scriptContent = fs.readFileSync(path.join(__dirname, 'remote-diagnose-email.js'), 'utf8');
...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    ...
    const remoteScriptPath = '/home/digitalbordados/digitalbordados/scripts/remote-diagnose-email.cjs';
    const writeStream = sftp.createWriteStream(remoteScriptPath);

    writeStream.on('close', () => {
      console.log('Script enviado. Executando...\n');
      conn.exec(`cd /home/digitalbordados/digitalbordados && node scripts/remote-diagnose-email.cjs`, (err2, stream) => {
        if (err2) { console.error('Exec err:', err2); conn.end(); return; }
        stream.on('close', (code) => {
          console.log(`\nCódigo: ${code}`);
          conn.exec(`rm -f /home/digitalbordados/digitalbordados/scripts/remote-diagnose-email.cjs`, () => conn.end());
        }).on('data', (data) => {
